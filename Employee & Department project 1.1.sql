...
-- name starts with 'S'
SELECT * FROM Employees WHERE FirstName LIKE 'S%' OR LastName LIKE 'S%';
...
SELECT DepartmentID, min(Salary) AS MminSalary FROM Employees GROUP BY DepartmentID;
...
-- for employees earning more than ₹60,000 in each department. 
 ...
 SELECT DepartmentID, SUM(Salary) AS TotalHighSalary FROM Employees WHERE Salary > 60000 GROUP BY DepartmentID;

-- Situation -8:
--  To monitor departmental growth, count the number of employees who joined in 
-- 2024 in each department. 
SELECT DepartmentID, COUNT(*) AS EmployeesJoined2024 FROM Employees WHERE YEAR(DateOfJoining) = 2024 GROUP BY DepartmentID;

-- Situation -9:
--  For training allocation, count the number of employees with salaries between 
-- ₹50,000 and ₹70,000 in each department. 
SELECT DepartmentID, COUNT(*) AS MidRangeEmployees FROM Employees WHERE Salary BETWEEN 50000 AND 70000 GROUP BY DepartmentID;
...
-- Situation -4: 
-- For creating a unselect ique employee ID, concatenate the department ID and employee 
-- number. 
SELECT CONCAT(DepartmentID, EmployeeID) AS EmployeeID FROM Employees; 
-- Situation -5:
...
SELECT 
  CONCAT(
    Phone, 
    IF(Extension IS NULL OR Extension = '', '', CONCAT(' ext ', Extension))
  ) AS ContactNumber 
FROM Employees; 
-- Situation -6: 
...
select concat(DepartmentName, ' - ', Position) AS Notification FROM Employees JOIN Departments ON Employees.DepartmentID = Departments.DepartmentID;
?
- Situation -9: 
...
SELECT CONCAT(@year, FirstName) AS ReferenceCode FROM Employees;
...
delete from employees 
WHERE DepartmentID = 2 AND 'JoinDate' < '2022-01-01';
...
-- 2021. 
DELETE FROM Employees 
WHERE DepartmentID = 3 AND 'JoinDate' < '2021-01-01';
...
DELETE FROM Employees 
WHERE 'Region' = 'North' ANd 'EndDate' < '2023-01-01';
